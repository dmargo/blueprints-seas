...
public class DupEdge extends DupElement implements Edge {
	...
	final private static String DEFAULT_LABEL = "DUP_DEFAULT_LABEL";
		...
		final String label)
    {
        ...
        DupEdgeData edata = new DupEdgeData();
        edata.label = label != null ? label : DupEdge.DEFAULT_LABEL;
        edata.id = this.in;
        DupEdge.edgeDataBinding.objectToEntry(edata, graph.data);
        
        try {
        	status = graph.outDb.putNoDupData(null, outVertex.id, graph.data);
        	if (status == OperationStatus.SUCCESS) {
        		edata.id = this.out;
        		DupEdge.edgeDataBinding.objectToEntry(edata, graph.data);
        		status = graph.inDb.putNoDupData(null, inVertex.id, graph.data);
    ...
    protected DupEdge(final DupGraph graph, final Object id) {
    	if(id.getClass() != DupEdgeKey.class)
    		throw new RuntimeException("DupEdge: " + id + " is not a valid Edge ID.");
		...
		DupEdgeData edata = new DupEdgeData();
		edata.label = ekey.label;
		edata.id = ekey.in;
		DupEdge.edgeDataBinding.objectToEntry(edata, graph.data);

        try {
        	status = graph.outDb.getSearchBoth(null, graph.key, graph.data, null);
        } catch (RuntimeException e) {
			throw e;
		} catch (Exception e) {
			throw new RuntimeException(e.getMessage(), e);
    ...
    protected void remove() {
    	...
    	DupEdgeKey ekey = new DupEdgeKey();
    	ekey.out = this.out;
    	ekey.label = this.label;
    	ekey.in = this.in;
        ...
        DupEdgeData edata = new DupEdgeData();
        edata.label = this.label;
        edata.id = this.in;
        DupEdge.edgeDataBinding.objectToEntry(edata, this.graph.data);
        
        try {
        	cursor = this.graph.outDb.openCursor(null, null);
